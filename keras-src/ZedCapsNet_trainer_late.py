...
def train(save_dir, batch_size, lr, lr_decay_value, lam_recon, shift_fraction, epochs, model, data, running_time):
    ...
    log_dir = save_dir + '/tensorboard-logs' + '/' + running_time
...
shift_fraction=0.1 # Fraction of pixels to shift at most in each direction

test_data_size = True

load_dir=os.path.join('../','tensors/fig1-2')
save_dir=os.path.join(load_dir,'new_report',running_time)
...
# load data
x_train, h5file = h5_read(os.path.join(load_dir,"data_train.hdf5"))
h5file.close()
y_train, h5file = h5_read(os.path.join(load_dir,"labels_train.hdf5"))
h5file.close()
y_train = y_train.flatten()

x_validate, h5file = h5_read(os.path.join(load_dir,"data_validate.hdf5"))
h5file.close()
...
model.summary()
model.load_weights('../models/late.h5')
